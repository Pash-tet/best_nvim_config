...
- **`opt.confirm = true`** напрашивается как очевидный фикс E37 из
  ...
  и так редкость.
- ruby-lsp и rubocop вне mason из-за asdf-мультиверсионности; строгие
  `root_dir` для tailwind/stimulus/ts_ls (гейт по фактической установке
  пакета, а не по наличию конфигов) — продумано лучше, чем в типовых
  конфигах, не трогать.
